...
export const UserProducts = () => {
    ...
    const {
      ...
      const fetchData = async () => {
        try {
          startLoading();
          const {data : places} = await axios.get(`${FetchRoutes.BASEURL}${PrivateRoutes.USERPRODUCTSID(user.id)}`,
    ...
    const deleteProduct = async (pId) => {
        try {
          startLoading();
          await axios.delete(`${FetchRoutes.BASEURL}/product/${pId}`,
          { headers: { 
     ...
     <>
        ...
        >
            <div className='flex items-end justify-between w-full py-6'>
                <h1 className='text-2xl font-semibold text-white'>My places</h1>
                <ArrowLeftIcon
                onClick={() => navigate(-1)}
        ...
        >
            ...
            grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8'>
                {data?.map((place, i) => (
                    <UserProductsCard data={place} deleteProduct={deleteProduct} key={i} />
                ))}
            </div>}
            {data?.length === 0 && 
            ...
            flex flex-col items-center p-4 mx-auto'>
             ...
             <p className='text-xl md:text-2xl font-medium mb-2 text-gray-800'>No places yet</p>
             <p className='md:text-lg text-gray-600 mb-4'>Search a place to start your journey</p>
             <Link
             to={PrivateRoutes.PRODUCTCREATE}
